...
const nextConfig = {
  ...
  async rewrites() {
    if (process.env.BUILD_FOR_CF === '1') return [] // static export: no rewrites; client uses NEXT_PUBLIC_API_URL
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'
    return [
      // Handle locally so we don't get 500 from backend (no GET /api/mission/templates or GET /api/civilian/detect)
    ...
    return [
      {
        ...
        headers: [
          ...
          },
          {
            ...
            value: "frame-ancestors 'self'; default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline' blob: https:; script-src-elem 'self' 'unsafe-inline' blob: https:; style-src 'self' 'unsafe-inline' https:; style-src-elem 'self' 'unsafe-inline' https:; img-src 'self' data: https: blob:; font-src 'self' data: https:; connect-src 'self' ws: wss: http: https:; frame-src 'self' https:; worker-src 'self' blob:;",
          },
        ],
      },
    ]
